...
int       proxyEntity[MAXPLAYERS + 1];               // Used to track current particles created by our plugin, when player no longer needs them, we call DeleteParticle to ensure entity limit is not reached, see above.
// int 	  particleEntity[MAXPLAYERS+1];
...
void FetchWearablesHandler(Database db, DBResultSet results, const char[] error, any data)
{
    ...
    while (results.FetchRow())
    {
        ...
        if (!SQL_IsFieldNull(results, 0))
            player.SetKillstreakTierId(results.FetchInt(0), slot1);

        // primarySheen
        if (!SQL_IsFieldNull(results, 1))
            player.SetKillstreakSheenId(results.FetchInt(1), slot1);

        // primaryEffect
        if (!SQL_IsFieldNull(results, 2))
            player.SetKillstreakEffectId(results.FetchInt(2), slot1);

        // unusualPrimary
        if (!SQL_IsFieldNull(results, 11))
            player.SetUnusualWeaponEffectId(results.FetchInt(11), slot1);

        // secondaryTier
        if (!SQL_IsFieldNull(results, 3))
            player.SetKillstreakTierId(results.FetchInt(3), slot2);

        // secondarySheen
        if (!SQL_IsFieldNull(results, 4))
            player.SetKillstreakSheenId(results.FetchInt(4), slot2);

        // secondaryEffect
        if (!SQL_IsFieldNull(results, 5))
            player.SetKillstreakEffectId(results.FetchInt(5), slot2);

        // unusualSecondary
        if (!SQL_IsFieldNull(results, 12))
            player.SetUnusualWeaponEffectId(results.FetchInt(12), slot2);

        // meleeTier
        if (!SQL_IsFieldNull(results, 6))
            player.SetKillstreakTierId(results.FetchInt(6), slot3);

        // meleeSheen
        if (!SQL_IsFieldNull(results, 7))
            player.SetKillstreakSheenId(results.FetchInt(7), slot3);

        // meleeEffect
        if (!SQL_IsFieldNull(results, 8))
            player.SetKillstreakEffectId(results.FetchInt(8), slot3);

        // unusualMelee
        if (!SQL_IsFieldNull(results, 13))
            player.SetUnusualWeaponEffectId(results.FetchInt(13), slot3);
...
void UpdateWearables(int client, char[] steamid)
{
    ...
    int    primary   = TF2Econ_TranslateLoadoutSlotNameToIndex("primary");
    int    secondary = TF2Econ_TranslateLoadoutSlotNameToIndex("secondary");
    int    melee     = TF2Econ_TranslateLoadoutSlotNameToIndex("melee");
    ...
    FormatEx(query, sizeof(query), "UPDATE %s SET primaryTier='%i', primarySheen='%i', primaryEffect='%i', unusualPrimary='%i', secondaryTier='%i', secondarySheen='%i', secondaryEffect='%i', unusualSecondary='%i', meleeTier='%i', meleeSheen='%i', meleeEffect='%i', unusualMelee='%i', unusualTauntId='%s', unusualHatId='%i' WHERE steamid='%s'", buffer, player.GetKillstreakTierId(primary), player.GetKillstreakSheenId(primary), player.GetKillstreakEffectId(primary), player.GetUnusualWeaponEffect(primary), player.GetKillstreakTierId(secondary), player.GetKillstreakSheenId(secondary), player.GetKillstreakEffectId(secondary), player.GetUnusualWeaponEffect(secondary), player.GetKillstreakTierId(melee), player.GetKillstreakSheenId(melee), player.GetKillstreakEffectId(melee), player.GetUnusualWeaponEffect(melee), effect[client], player.GetUnusualHatEffectId(), steamid);
    WearablesDB.Query(SQLError, query);
}
...
public void TF2_OnConditionRemoved(int client, TFCond condition)
{
    ...
    if (IsValidEntity(proxyEntity[client]) && proxyEntity[client] != 0)
    {
        AcceptEntityInput(proxyEntity[client], "ClearParent");
        TeleportEntity(proxyEntity[client], { 1000.0, 1000.0, 1000.0 }, NULL_VECTOR, NULL_VECTOR);

        DataPack data = new DataPack();
        data.WriteCell(proxyEntity[client]);
        RequestFrame(RequestDelete, data);
    }

    delete refireTimer[client];    // Stop timer from refiring if player is no longer taunting.
}

public void RequestDelete(DataPack pack)
{
    pack.Reset();
    int proxy = pack.ReadCell();

    ClearTempParticles(proxy);
    ...
    switch (menuAction)
    {
        case MenuAction_Select:
        {
            ...
            if (StrEqual(info, "Primary"))
            {
                ...
                int wep  = ProcessLoadoutSlot(slot, client);
...
stock int AttachParticle(int entity, char[] particle)
{
    ...
    if (IsValidEntity(particleSystem))
    {
        ...
        GetEntPropVector(entity, Prop_Send, "m_vecOrigin", pos);    // Update position vector with default position values
...
// Labelled as stock as we are not currently using this in the plugin, but have plans to in the future. (Prevents compiler warnings.)
public void DeleteParticle(int particle)
{
...
void CreateTempParticle(char[] particle, int entity = -1, float origin[3] = NULL_VECTOR, float angles[3] = { 0.0, 0.0, 0.0 }, bool resetparticles = false)
{
    ...
    TE_WriteNum("m_iAttachType", PATTACH_ABSORIGIN_FOLLOW);    // NEW: AttachType set to AbsOrigin and set to follow proxy entity
...
void ClearTempParticles(int entity)
{
    CreateTempParticle("sandwich_fx", entity, _, _, true);    // Creates a empty sandwich_fx particle and passes true on m_bResetParticles, deleting all particles attached to entity.
}
...
public Action ProcessWeapons(int client, int itemIndex, Handle &hItem)
{
    Player player      = Player(client);
...
int CreateTauntEntity(int client, char[] effect)
{
    int iTaunt = CreateEntityByName("tf_taunt_prop");

    if (!IsValidEdict(iTaunt))
        return -1;

    DispatchSpawn(iTaunt);
    ...
    SetEntityModel(iTaunt, sModel);

    SetEntPropEnt(iTaunt, Prop_Data, "m_hOwnerEntity", client);
    SetEntPropEnt(iTaunt, Prop_Data, "m_hEffectEntity", client);
    SetEntProp(iTaunt, Prop_Send, "m_fEffects", EF_BONEMERGE | EF_NOINTERP | EF_NORECEIVESHADOW | EF_PARENT_ANIMATES);

    SetVariantString("!activator");
    AcceptEntityInput(iTaunt, "SetParent", client);
